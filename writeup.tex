...
The layout was only done for the resistor-MOSFET voltage divider.
...
\item 0.18$\mu$m CMOS technology from the Analog Integrated Circuit Design 2nd Ed. course website%\footnote{\href{http://analogicdesign.com/students/netlists-models/model-files/}{Analog Integrated Circuit Design 2nd Ed. Model Files}} website
...
The main objectives for the voltage reference are
\begin{enumerate}
\item Minimize temperature coefficient
\item Minimize $V_{DD}$ dependence
\end{enumerate}
These will be achieved through design calculations and using an iterative approach to obtain the goals.

\subsection{Design Topologies}
Four different voltage reference topologies were investigated.
\subsubsection{Design 1. Resistor-MOSFET Voltage Divider}
The most basic design.  It should be good up to the first-order model but performance may suffer when higher-order parameters are included.
...
\subsubsection{Design 3. Current-Diode Divider}
...
Fig. \ref{resistor_circuit1} shows the first iteration of the resistor-MOSFET circuit.  
\subsubsection{Calculations}
The total current through the N-channel MOSFET transistor is
\begin{equation}
\label{R_totalcurrent}
I_{_D} = \frac{V_{_{DD}}-V_{_{REF}}}{R}
\end{equation}

The transistor drain current is
\begin{equation}
\label{R_draincurrent}
I_{_D} = \frac{1}{2}K_{_n}(V_{_{REF}}-V_{_{TN}})^2
\end{equation}

Using Eq. \ref{R_totalcurrent} and Eq.\ref{R_draincurrent} and solving for the reference voltage gives
\begin{equation}
\label{R_ref}
...
Fig. \ref{resistor_circuit2} adds an ideal buffer to the output to shield the circuit from loading effectst and leakage current.  Note: These assumptions were also used in Iteration 1 so there are no changes in the calculations.
%\subsubsection{Calculations}
%\subsubsection{Simulation}
%\subsubsection{Analysis}
...
\bibitem{IEEEhowto:analogcarusone}
Add the Analog Integrated Circuit textbook here
\bibitem{IEEEhowto:cmosbaker}
Add the CMOS Design book here
